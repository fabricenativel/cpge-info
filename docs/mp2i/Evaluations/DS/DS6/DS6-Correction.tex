...
\tcor{
  ...
  Pour la méthode diviser pour régner, en notant $C(n)$ le nombre d'opérations pour un tableau de taille $n$, on a $C(n) = 2C(\dfrac{n}{2}) + K$ où $K$ est une constante indépendante de $n$ en effet, la résolution fait deux appels récursifs sur des tableaux de taille $\dfrac{n}{2}$ et des opérations élémentaires. A partir de cette équation on montre que $C(n) \in O(n)$, donc la méthode diviser pour régner a une complexité linéaire.
}
\end{Exercise}
