...
def make_tex_eleve(eleve,notes,rthemes,rexos):
...
def make_tex(nds,dateds,notes,rthemes,rexos):
    writer = open(DS+str(nds)+"-Resultats.tex","w")
    writer.write(ENTETE)
    writer.write(f"\RDS{{{nds}}}{{{dateds}}}\n")
    for e in eleves:
        writer.write(make_tex_eleve(e,notes,rthemes,rexos))
...
if not os.path.isfile(ODS_FILE):
    ...
    make_tex(nds,dateds,notes,resultat_theme,resultat_exo)
    for e in dnotes:
