...
\begin{Exercise}[title = {Hachage de chaine de caractères}]
	...
	$\displaystyle{h(s) = \sum_{i=0}^{n-1} N(c_i) * 32^i}$\\
...
\Question{Ecrire la fonction $f_N$ en OCaml avec la signature : \mintinline{ocaml}{let f int -> int -> int-> int * int} en donnant les paramètres dans cet ordre : {\tt x}, {\tt y} et {\tt N}.}
\Question{Ecrire une fonction \mintinline{ocaml}{let appartient 'a -> 'a list -> bool} qui teste si un élément appartient à une liste. Donner la complexité de cette fonction en fonction de la taille de la liste.}
\Question{Ecrire une fonction \mintinline{ocaml}{let cycle_naif int -> int -> int -> int * int} qui prend en argument (dans cet ordre) $x_0$, $y_0$ et $N$ et détecte le cycle en maintenant à jour une liste des couples déjà rencontrés, et renvoie le premier couple qui apparaît deux fois.}
...
\smallskip
On propose maintenant d'utiliser une table de hachage afin de stocker les termes déjà rencontrés, les clés sont les couples rencontrées et la valeur associée est l'indice dans la suite de ce couple. Par exemple si les trois premiers termes de la suite sont $(1,5), (4,3), (7, 7)$ alors la table de hachage doit contenir les clés $(1,5), (4,3)$, et $(7,7)$ associées respectivement aux valeurs $0, 1$ et $2$.
