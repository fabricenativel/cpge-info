...
* $C(x,y) = 0$ lorsque $x$ peut indifférent être placé avant ou après $y$ ($x$ et $y$ sont "égaux" pour l'ordre considéré).
...
!!! Exemple
    Supposons qu'on trie la liste de points du plan $[(3;6), (2;5), (1;4), (2;3)]$ par abscisse croissante, alors les points $(2;5)$ et $(2;3)$ ayant même abscisse peuvent être placés indifféremment l'un avant ou après l'autre dans le tri, c'est à dire que :
...
On remarque donc que ce tri n'est *pas stable* comme indiqué dans la [documentation officielle de OCaml](https://ocaml.org/manual/5.3/api/Array.html#sorting_and_shuffling){target=_blank}. D'autre part, on notera que l'implémentation actuelle de ce tri se base sur le [tri par tas](https://fr.wikipedia.org/wiki/Tri_par_tas){target=_blank} déjà rencontré en cours.
