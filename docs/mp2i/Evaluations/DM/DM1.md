...
1. Ecrire une fonction `retourne` qui prend en argument un tableau `tab`, de taille `t` retourne ce tableau . Par exemple si `tab = { 2, 7, 8, 9}` alors après l'appel `retourne(tab, 4)`, le contenu de `tab` sera `{ 9, 8, 7, 2}`.
