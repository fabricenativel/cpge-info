...
* une *clause* par une liste d'entiers
...
2. Choix d'une variable à instancier  
    ...
    * Le choix du littéral dans `[[3]; [1; 2; 3; -5]; [-2]; [-1; 5]]` peut renvoyer `3` ou `-2` (on renvoie l'un des littéraux figurant dans une clause de longueur minimale)
...
1. Présence d'au moins une valeur dans chaque case du Sudoku
    ...
    Chaque case de la grille doit contenir une valeur. C'est à dire que, par exemple, pour la case $(0,0)$ on doit avoir l'un des $p_{0,0,k}$ pour $1 \leqslant k \leqslant 9$ qui est vrai. Ce qui donne pour la case $(0,0)$ la clause `[1; 2; 3; 4; 5; 6; 7; 8; 9]`. Ecrire la fonction  `au_moins : () -> int list list` qui ne prend aucun argument et renvoie la *fnc* indiquant que chacune des 81 cases du sudoku contient au moins une valeur.

2. Présence d'au plus une valeur dans chaque case du Sudoku

    Une case du Sudoku ne peut pas contenir deux valeurs simultanément. Prenons de nouveau l'exemple de la case $(0,0)$, pour deux valeurs distinctes $k_1$ et $k_2$ on ne peut pas avoir simultanément les variables $p_{0,0,k_1}$ et $p_{0,0,k_2}$ vraies, autrement dit l'une ou l'autre doit être fausse. C'est à dire qu'on obtient, toujours dans l'exemple de la case $(0,0)$, que pour toute couple $(k_1,k_2)$, $1 \leqslant k_1 < k_2 \leqslant 9$ la clause `[-k1; -k2]`. Ecrire une fonction `au_plus : () -> int list list` qui ne prend aucun argument et renvoie la *fnc* indiquant que chacune des 81 cases du sudoku contient au plus une valeur.
...
3. Valeur différentes dans une ligne, une colonne ou un bloc
    ...
    3. Ecrire la fonction  `bloc : int -> int list` qui renvoie la liste des cases du bloc `i` (on peut numéroter les bloc du sudoku de gauche à droite et de haut en bas)
...
3. Ecrire une fonction exploitant la valuation afin d'afficher le sudoku résolu.
