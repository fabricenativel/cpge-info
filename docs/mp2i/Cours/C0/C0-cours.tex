	...
	\begin{block}{Modèle de Von Neumann}
		\begin{itemize}
			...
			\item<2-> Dans ce modèle, l'ordinateur se décompose :
				\begin{itemize}
					\item<3-> La \textcolor{blue}{mémoire} qui stocke les données et les programmes
					\item<4-> L'\textcolor{blue}{unité arithmétique et logique {\sc ual}} qui effectue les opérations (addition, soustraction, comparaison, \dots) sur les données.
	...
	\begin{block}{Généralités sur le {\tt shell}}
		\begin{itemize}
			...
			\item<3-> Le résultat d'une commande peut-être dirigé :
				\begin{itemize}
					\item<7-> Vers un fichier avec \textcolor{Sepia}{\tt >} (avec écrasement si le fichier existe)
					\item<8-> Vers un fichier avec \textcolor{Sepia}{\tt >{}>} (avec ajout en fin si le fichier existe)
					\item<9-> Vers une autre commande avec \textcolor{Sepia}{\tt |} (\textit{pipe})
				\end{itemize}
		\end{itemize}
	\end{block}
\end{frame}
