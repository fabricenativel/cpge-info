...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Le problème de la représentation des données}
		\begin{itemize}
			\item<1-> La mémoire d'un ordinateur est composée de \textit{bits} pouvant prendre uniquement les valeurs 0 et  1
			\item<2-> Le regroupement de 8 bits s'appelle un octet (\textit{byte} en anglais), c'est l'unité minimale de mémoire :
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{De la base 10 à la base 2}
		\begin{itemize}
			\item<1-> Nous sommes habitués à écrire les entiers positifs en utilisant \textcolor{red}{10} chiffres, chaque chiffre étant  multiplié par une puissance de \textcolor{red}{10} suivant son emplacement dans le nombre.\\
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{De la base 10 à la base 2}
		\begin{itemize}
			\item Nous sommes habitués à écrire les entiers positifs en utilisant \textcolor{red}{10} chiffres, chaque chiffre étant  multiplié par une puissance de \textcolor{red}{10} suivant son emplacement dans le nombre.\\
	...
	\begin{exampleblock}{Exemples}
		...
		\begin{itemize}
			...
			\item<4-> $\base{3EA}{16}$
				...
				\onslide<5->\textcolor{gray}{\small On travaille ici en base 16, donc avec 16 chiffres, les lettres majuscules de $A$ à $F$ représentent les \og{} chiffres \fg{} 10 à 15.}  \\
		\end{itemize}
	\end{exampleblock}
\end{frame}
	...
	\begin{block}{Limitations mémoire et dépassement de capacité}
		\begin{itemize}
			...
			\item<2-> En C, les valeurs maximales représentables suivant le type d'entier positif utilisé sont donc :
				\begin{itemize}
					\item<2-> \kw{unsigned int} : \textit{au min 16 bits, usuellement 32 bits, dépendant du compilateur}
				...
				\onslide<7->{Par exemple, les dépassements de capacité sur un \kw{uint8\_t} sont calculés modulo 256.}
	...
	\begin{block}{Complément à deux}
		\begin{itemize}
			...
			\item<2-> La méthode utilisée est celle du complément à 2, sur $n$ bits, on compte négativement le bit de poids $2^{n-1}$ et positivement les autres. \\
				\onslide<3->{Par exemple, sur 8 bits : $\begin{array}{|c|c|c|c|c|c|c|c|} \hline \textcolor{BrickRed}{1}& 0 & 0 & 1 & 1 & 0 & 1 & 1\\ \hline \end{array} = \textcolor{BrickRed}{-2^7} + 2^4 + 2^3 + 2^1 + 2^0 = -101$}
			\item<3-> De façon générale, sur $n$ bits, la valeur en \textcolor{blue}{complément à deux} de la suite de bits $(b_{n-1}\dots b_0)$ est :
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Conséquences de la représentation en complément à 2}
		\begin{itemize}
			...
			\item<2-> Le plus petit entier représentable sur $n$ bits est alors $-2^{n-1}$ et le plus grand $2^{n-1}-1$
			\item<3-> En C, les valeurs extrêmes représentables sont :
				\begin{itemize}
					\item<4-> \kw{int} : \textit{au min 16 bits, usuellement 32 bits, dépendant du compilateur}
					\item<5-> \kw{int8\_t} : $\intN{-2^7}{2^7-1} = \intN{-128}{127}$
					\item<6->  \kw{int32\_t} : $\intN{-2^{31}}{2^{31}-1}$
					\item<7->  \kw{int64\_t} : $\intN{-2^{63}}{2^{63}-1}$
				\end{itemize}
			...
			\item<9-> En OCaml, les entiers sont codés sur 64 bits mais un bit est réservé par le langage, l'intervalle représentable est donc $\intN{-2^{62}}{2^{62}-1}$.\\
				\onslide<10-> Les dépassements de capacité sont calculés modulo  $2^{63}$ puis ramenés dans l'intervalle précédent.
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Correction}
		\begin{enumerate}
			\item<1->\textcolor{OliveGreen}{En complément à 2 sur 8 bits, $\base{10110001}{2} = -2^{7}+2^5+2^4+2^0 = -79$}
			\item<2->\textcolor{OliveGreen}{Ecriture en complément à 2 sur 8 bits de $-12$.} \\
			      \begin{tabular}{ll}
				      ...
				      \onslide<4->\textcolor{OliveGreen}{\textbf{2.} On inverse tous les bits :}                 & \onslide<5->\textcolor{OliveGreen}{$11110011$} \\
			...
			\item<8->\textcolor{OliveGreen}{Ecriture en complément à 2 sur 8 bits de $-75$.} \\
			      \begin{tabular}{ll}
				      ...
				      \onslide<11->\textcolor{OliveGreen}{\textbf{2.} On inverse tous les bits :}                    & \onslide<12->\textcolor{OliveGreen}{$10110100$} \\
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Algorithme des divisions successives}
		\begin{itemize}
			\onslide<1->{\item L'algorithme des \textcolor{blue}{divisions successives} permet d'écrire un nombre donné en base 10 dans n'importe quelle base $b$. Le principe est d'effectuer les divisions euclidiennes successives par $b$, les restes de ces divisions sont les chiffres du nombre dans la base $b$.}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple d'utilisation de l'algorithme des divisions successives}
		...
		\end{tabular} \\
		\onslide<14->{
			Le quotient est nul, l'algorithme s'arrête et les chiffres en base 16 sont les restes obtenus à chaque étape donc  $\base{9787}{10}=\base{263B}{16}$ (car 11 correspond au chiffre B).}
	\end{exampleblock}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	...
	\begin{algorithm}[H]
	  ...
	  \end{algorithm}
	  \onslide<2->{Prouver que cet algorithme est correct.}
	  \onslide<3->{\textcolor{gray}{Une implémentation en C sera vue en TP.}}
	\end{block}
\end{frame}


\makess{Représentation des caractères}
\begin{frame}{\Ctitle}{\stitle}
	\begin{alertblock}{Principe}
		L'idée de base est d'associer à chaque caractère un entier positif : son \textit{code}.
	\end{alertblock}
	\begin{block}{Codages standards}
		\begin{itemize}
		\item<2-> La première tentative de standardisation est le codage {\sc ascii} (\textit{American Standard Code for Information Interchange}) qui associe à chaque entier représenté sur \textcolor{BrickRed}{7} bits (donc de 0 à 127), un caractère et qui respecte l'ordre \textit{lexicographique}. Par exemple : \\
		\begin{tabular}{|p{2cm}|p{1cm}|p{5cm}|}
			\hline
			Caractère & Code & Remarque\\
			\hline
			espace & 32 &  $\base{0100000}{2}$ (1\textsuperscript{er} caractère affichable)\\
			{\tt 0} & 48 & $\base{0110000}{2}$\\
			{\tt A} & 65 & $\base{1000001}{2}$\\
			{\tt a} & 97 & $\base{1100001}{2}$\\
			\hline
			\end{tabular}
		 \item<3-> Ce code incomplet pour de nombreuses langues (par exemple, on n'y trouve pas les caractères accentués comme {\tt é, à, ù, \dots}) a été \textcolor{blue}{étendu} dans le standard \textcolor{blue}{Unicode} (notamment via l'encodage {\sc utf}-8 \textit{Unicode Transformation Format}).
		\end{itemize}
	\end{block}
\end{frame}

\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Cas du langage C et de OCaml}
		\begin{itemize}
		\item<1-> En langage C, nous utiliserons exclusivement le type \kw{char}  qui permet de représenter \textcolor{BrickRed}{uniquement} les caractères {\sc ascii}.
		\item<2-> Avec cette représentation, on ne distingue pas un caractère de son code. Le type {\tt char} correspond en fait au type du plus petit entier représentable. Par exemple \mintinline{c}{char c = 'A' + 2;} est valide et {\tt c} contient alors \mintinline{c}{'C'}.
		\item<3-> Les comparaisons sont aussi possibles, on compare en réalité les codes (qui respectent l'ordre lexicographique). Par exemple \mintinline{c}{'A' < 'G'} vaut \mintinline{c}{true}.
		\item<4-> Le cas de OCaml sera vu lors de l'étude de ce langage.
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Exercices}
		\begin{itemize}
		\item La ligne suivante \mintinline{c}{char c = 'é';} provoque un \textit{warning} à la compilation, pourquoi selon vous ?
		\item Ecrire une fonction de signature \mintinline{c}{bool est_lettre(char c)} qui renvoie {\tt true} ssi {\tt c} est une lettre de l'alphabet.
		\item Écrire une fonction de signature \mintinline{c}{int chiffre_vers_int(char c)} qui prend en paramètre un caractère entre {\tt '0'} et {\tt '9'} et renvoie sa valeur numérique entière (ex : {\tt '7'} $\to$ {\tt 7}).
		\end{itemize}
	\end{exampleblock}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Ecriture dyadique}
		De la même façon que les chiffres après la virgule d'un nombre en écriture décimale utilisent les puissances de 10 négatives, par exemple \smallskip:
		\onslide<2->{
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Méthode : du décimal au dyadique}
		Pour traduire une partie fractionnaire en écriture dyadique :
		\begin{itemize}
			\item<2-> Multiplier la partie fractionnaire par 2. Si ce produit est supérieur ou égal à 1, ajouter 1 à l'écriture dyadique sinon ajouter 0.
			\item<3-> Recommencer avec la partie fractionnaire de ce produit tant qu'elle est non nulle.
		\end{itemize}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemples}
		\begin{enumerate}
			...
			\item<3-> Donner l'écriture dyadique de $3,5$\\
				\onslide<4-> \textcolor{OliveGreen}{$\base{3,5}{10}=\base{11,1}{2}$}
            \item<5-> Donner l'écriture dyadique de $0,1$
				\onslide<6->
                    \begin{enumerate}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{\textcolor{yellow}{\small \rappel} Ecriture scientifique}
		...
		\begin{itemize}
			\item<3-> avec $a \in [1;10[$, appelée \textcolor{blue}{mantisse} (l'écriture  décimale de $a$ n'a qu'un seul chiffre non nul à gauche de la virgule)
			\item<4-> et $n \in \Z$ appelé \textcolor{blue}{exposant}.
		\end{itemize}
	\end{block}
	\begin{exampleblock}{Exemples}
		\begin{itemize}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{alertblock}{Virgule flottante}
		...
		\begin{itemize}
            ...
            \item<4->L'exposant est décalé de façon à toujours être stocké sous la forme d'un entier positif. \textcolor{gray}{Ce décalage est de 127(=$2^7-1$) pour le format 32 bits et de 1023(=$2^{10}-1$) pour le format 64 bits.}
...
\begin{frame}{\Ctitle}{\stitle}
    \begin{exampleblock}{Exemple 2}
        ...
        \begin{enumerate}
            ...
            \item<6-> La mantisse est infinie, on la limite aux 23 premiers bits (c'est un arrondi et non une troncature). Le 1 initial  n'est pas stocké afin d'économiser un bit. La mantisse est donc \textcolor{OliveGreen}{$\boxed{\numprint{10011001100110011001101}}$}
        \end{enumerate}
...
\begin{frame}{\Ctitle}{\stitle}
    \begin{alertblock}{\textcolor{yellow}{\important} Attention !}
        ...
        \begin{itemize}
            ...
            \item<4-> Les valeurs calculées par un programme peuvent être très éloignées des valeurs théoriques d'un algorithme.\\
