...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Introduction}
		\begin{itemize}
			...
			\item<6-> Le C offre la possibilité de manipuler les variables mais aussi leurs adresses mémoires :
			\begin{itemize}
			\item<7-> \mintinline{c}{&x} (de type pointeur vers un entier : \mintinline{c}{int *}) est l'adresse de la variable {\tt x} (dans l'exemple précédent {\tt \&x = 2741})
			\item<8-> \mintinline{c}{*p} (de type entier : \mintinline{c}{int})est le contenu de l'adresse {\tt p} (dans l'exemple précédent {\tt *p = 42})
			\end{itemize}
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{alertblock}{{\tt free}}
		\begin{itemize}
			...
			\item<2-> On appelle donc \kw{free} sur un pointeur \kw{p} crée  par \kw{malloc}. Cet appel doit impérativement se faire sur la portée de la variable \kw{p}. En dehors, le bloc mémoire n'est plus libérable, ce qui conduit nécessairement à une fuite mémoire.
			\item<3-> Des blocs mémoire non libérés (ou non libérables) ne sont pas considérés comme disponibles et donc sont "perdus", ce sont les fameuses fuites mémoires (\textit{memory leak}), c'est à dire à une dimunition de la mémoire disponible durant le déroulement d'un programme.
			\item<4-> On utilisera toujours l'option \kw{fsanitize = adress} du compilateur pour détecter ces fuites mémoires.
		\end{itemize}
	\end{alertblock}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple : fonction renvoyant un tableau}
		\begin{itemize} \item<1-> Ecrire une fonction {\tt make\_tab} qui prend en argument deux entiers {\tt size} et {\tt init} et renvoie un pointeur vers un tableau de {\tt size} cases initialisés à la valeur {\tt init}.
				\onslide<2->\inputpartC{/home/fenarius/Travail/Cours/cpge-info/docs/mp2i/files/C3/renvoie_tab.c}{}{\small}{4}{9}
