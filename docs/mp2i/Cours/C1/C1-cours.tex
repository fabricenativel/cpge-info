...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Quelques aspects du C}
		\begin{itemize}
			...
			\item<3-> Le langage C est \textcolor{blue}{typé} c'est à dire qu'une variable appartient à un type.
			\item<4-> Equipé d'une librairie standard : la \textcolor{blue}{libc}.
			\item<5-> Plus \textcolor{blue}{proche de la machine} que bien d'autres langages de haut niveau, ce qui induit une certaine efficacité.
			\item<6-> Souvent utilisé pour le développement de systèmes d'exploitation, de pilotes de périphériques, de logiciels embarqués,
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple de boucle}
		\inputC{/home/fenarius/Travail/Cours/cpge-info/docs/mp2i/files/C1/cours_ex2.c}{7}{\small}
	\medskip
...
\begin{frame}{\Ctitle}{\stitle}
	...
	\medskip
	Définition d'une fonction \kw{syracuse} qui prend comme paramètre un entier et renvoie un entier. 
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Tableaux}
		\begin{itemize}
			...
			\item<8->\textcolor{BrickRed}{\important} Un accès en dehors des bornes du tableau est un \textcolor{blue}{comportement indéfini}\\
	...
	\begin{block}{Chaines de caractères}
		\begin{itemize}
			...
			\begin{tabular}{|>{\tt}c|>{\tt}c|>{\tt}c|>{\tt}c|>{\tt}c|>{\tt}c|>{\tt}c|>{\tt}c|}
				...
				\hline
			\end{tabular}
			\item<3-> Avec \kw{printf}, on affiche une chaine de caractères à l'aide du spécificateur de format \kw{\%s}.
			\item<4-> Le module \kw{string.h} fournit des fonctions usuelles de manipulation de caractères, notamment :
			\begin{itemize}
				\item<5-> \kw{strlen} : renvoie la longueur de la chaine de caractères
				\item<6-> \kw{strcpy} : copie une chaine de caractères
				\item<7-> \kw{strcat} : concaténation de chaines de caractères
			\end{itemize}
		\end{itemize}
	\end{block}
\end{frame}
