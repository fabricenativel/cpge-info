...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{\textcolor{Yellow}{\small \danger \; } Quelques points de vigilance !}
		\begin{itemize}
			...
			\item<3-> Pour obtenir le résultat de la division décimale, l'une au moins des opérandes doit être du type {\tt float}.
			\item<4-> Contrairement à Python, Les entiers ont une taille maximale en C, et certaines opérations peuvent produire des \textit{dépassements de capacités}. Ce point sera etudié plus loin dans le cours.
			\item<5-> Le type \kw{char} ne permet de représenter qu'un unique caractère. Les chaines de caractères seront vues plus loin dans le cours.
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple}
		Ecrire en C une fonction de signature \mintinline{c}{void identite(int n)} qui ne prend pas d'argument et affiche la matrice identité d'ordre {\tt n} (c'est à dire qu'elle contient des 1 sur sa diagonale principale et des 0 partout ailleurs). On déclarera un tableau à deux dimensions dans la fonction et on le remplira correctement avant d'effectuer son affichage.
	\end{exampleblock}
\end{frame}
