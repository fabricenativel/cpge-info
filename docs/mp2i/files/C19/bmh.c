...
int derniere_occurrence(char c, char *motif)
{
    ...
    {
        if (motif[i]==c) {return strlen(motif)-1-i;}
    }
    return strlen(motif);
}
...
{
    if (argc!=3) {printf("Donner le motif suivi du nom de fichier dans lequel cherché !\n");}
    else
    {
        ...
        int nbocc;
        int nbcmp = 0;
        if (reader==NULL) {printf("Le fichier n'a pas été trouvé \n");} else
        {
