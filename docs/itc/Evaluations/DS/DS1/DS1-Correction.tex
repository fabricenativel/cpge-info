...
\begin{Exercise}[title = {Question de cours}] \\
    ...
    \tcor{Montrons que la propriété \og{} $P(k)$ : \textit{après $k$ tours de boucles, la variable {\tt p} contient $x^k$}\fg{} est un invariant de boucle.
    \begin{itemize}
        \item $P(0)$ est vrai puisqu'avant d'entrer dans la boucle {\tt p=0}$=x^0$.
        \item En supposant $P(k)$ vraie, {\tt p=}$=x^k$ et donc au tour de boucle suivant {\tt p}=$x^k*x$, donc {\tt p=}$x^{k+1}$.
    \end{itemize}
    Donc $P(k)$ est vrai pour tout $k \in \N$. Comme on effectue $n$ tour de boucles, à la sortie de la boucle $p=x^n$ et donc la fonction est correcte.
    }
	\Question{Ecrire une version récursive de cette fonction et prouver qu'elle termine.}
    \tcor{
        \inputpartPython{\SPATH /puissance.py}{}{}{10}{15}
        Cette fonction termine car {\tt n} est un variant, en effet :
        \begin{itemize}
            \item {\tt n} est à valeurs dans $\N$, puisque {\tt n}$>=0$ par précondition.
            \item {\tt n} décroît strictement, en effet {\tt n} diminue de 1 à chaque appel récursif.
        \end{itemize}
    }
\end{Exercise}
...
\inputpartPython{\SPATH /anagrammes.py}{}{}{1}{12}
...
\inputpartPython{\SPATH /anagrammes.py}{}{}{14}{21}
...
\tcor{On effectue un appel récursif pour chaque caractères de la {\tt chaine1} donc au plus $n$ appels. Pour chacun de ses appels, on parcourt la {\tt chaine2} (donc au moins $n$ opérations). On total, la complexité est donc $O(n^2)$. }
...
\inputpartPython{\SPATH /anagrammes.py}{}{}{23}{32}
...
\inputpartPython{\SPATH /anagrammes.py}{}{}{34}{40}
...
\inputpartPython{\SPATH /anagrammes.py}{}{}{42}{45}
\Question{Donner (en la justifiant) la complexité de cette fonction en notant $n$ la taille de la plus longue des deux chaines.}
\tcor{On sait que le test d'appartenance à un dictionnaire est en $O(1)$, donc ici la création de chacun des deux dictionnaires est en $O(n)$ (car on n'a que des opérations en $O(1)$ dans la boucle). De même le test d'égalité entre les deux dictionnaires est en $O(n)$ et donc cette fonction est en $O(n)$.}
...
\inputpartPython{\SPATH /test_secu.py}{}{}{1}{7}
...
\inputpartPython{\SPATH /test_secu.py}{}{}{9}{10}
...
\inputpartPython{\SPATH /test_secu.py}{}{}{12}{13}
...
\inputpartPython{\SPATH /test_secu.py}{}{}{15}{18}
...
\tcor{La liste des chiffres de rang impair est {\tt [2, 7]} et celle de rang pair est {\tt [6, 4]}. Après traitement, celle de rang pair devient {\tt [3, 8]}. On additionne l'ensemble des chiffres obtenus  : $2 + 7 + 3 + 8 = 20$, on obtient bien un entier divisible par 10, donc le numéro est valide.}
...
\inputpartPython{\SPATH /test_cc.py}{}{}{1}{5}
...
\inputpartPython{\SPATH /test_cc.py}{}{}{7}{10}
...
\inputpartPython{\SPATH /test_cc.py}{}{}{12}{19}
...
\inputpartPython{\SPATH /test_cc.py}{}{}{21}{30}
\end{Exercise}
