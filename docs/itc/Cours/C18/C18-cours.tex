...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Position du problème}
		...
		\begin{center}
			...
			\item<3-> {\small que penser d'une solution qui testerait toutes les possibilités ?\\} \onslide<7->\textcolor{OliveGreen}{\small La complexité serait exponentielle en fonction du nombre de niveau.}
			\item<4-> {\small obtient-on la solution optimale en choisissant l'entier maximal à chaque étape ?}
			\item<5-> {\small vérifier qu'on retrouve les caractéristiques d'un problème soluble par programmation dynamique.}
			\end{itemize}
	\end{exampleblock}
\end{frame}
