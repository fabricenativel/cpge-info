...
\begin{Exercise}[title = {Représentation en C}]\\
	...
	\inputpartOCaml{\SPATH/arbres_binaires.c}{}{}{7}{14}
	...
	\mintinline{c}{void libere(ab* arbrebinaire)} qui détruit l'arbre binaire donné en paramètre, en libérant l'espace alloué par ses noeuds. A la fin de l'appel {\tt arbrebinaire} est le pointeur {\sc null}.}
\end{Exercise}
...
\begin{Exercise}[title = {Représentation en OCaml}]\\
	...
	\inputpartOCaml{\SPATH/arbres_binaires_int.ml}{}{}{2}{4}
	\Question{Dessiner l'arbre représenté par :
		\inputpartOCaml{\SPATH/arbres_binaires_int.ml}{}{}{86}{94}
	}
...
\begin{Exercise}[title = {Un peu de dénombrement}]\\
	...
	{\small \aide}\ Revenir à la définition vue en cours et considérer un arbre binaire à $n$ noeud comme un arbre binaire ayant un sous arbre contenant $k$ noeuds et un sous arbre contenant $n-k-1$ noeud.}
...
\begin{Exercise}[title = {Un peu de complexité}]\\
	...
	\inputpartOCaml{\SPATH/arbres_binaires_int.ml}{}{}{48}{51}
	\Question{Ecrire une spécification et donner un nom plus approprié à la fonction {\tt mystere}.}
	...
	{\small \aide\;} Utiliser une fonction auxiliaire avec un accumulateur en \textit{et écrire la spécification de cette fonction}.
	}
\end{Exercise}
...
\begin{Exercise}[title = {Recherche dans un {\sc abr}}]
	...
	\Question{En fournir une implémentation en OCaml, en supposant que la séquence est donnée sous la forme d'une liste d'entiers de OCaml. La signature de votre fonction sera donc {\tt int list -> bool}}
	\Question{Soit $t$ une liste représentant la suite de valeurs obtenue lors de la recherche réussie d'un élément dans un {\sc abr}, proposer un algorithme \textit{de complexité linéaire} permettant de trier ce tableau. En donner l'implémentation en OCaml.}
\end{Exercise}

\begin{Exercise}[title = {Sur les tas}] \vspace{0.2cm}\\
	\begin{tabularx}{\textwidth}{Y|Y}
		...
		$T_1$ & $T_2$ \\
	\end{tabularx}
	\Question{Vérifier que $T_1$ est un tas binaire min.}
	\Question{Pour quelles valeurs de $x$, $T_2$ est-il un tas ?}
	\Question{Donner la représentation de $T_2$ par un tableau.}
	\Question{Dessiner un arbre binaire contenant les valeurs $6, 18, 10, 9$ et  $17$ telle que cet arbre ne soit \textit{pas} un tas et que l'étiquette de chaque noeud soit plus petite que les étiquettes de ses fils (lorsqu'elles existent).}
	\Question{Rappeler l'algorithme d'extraction du minimum d'un tas et l'appliquer à $T_1$ et donner l'arbre obtenu après extraction.}
	\Question{Rappeler l'algorithme d'insertion dans un tas et l'appliquer à $T_2$ pour insérer la valeur $28$ et donner l'arbre obtenu après cette insertion.}
\end{Exercise}
