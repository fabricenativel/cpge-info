...
\begin{Exercise}[title={Déclarations}]
	\Question{Écrire les instructions permettant de :}
		...
		\subQuestion{Déclarer une variable {\tt test} de type booléen, quelle bibliothèque (ou en-tête) est nécessaire ?}
		\subQuestion{Déclarer un tableau de 5 entiers initialisés aux valeurs \{1, 4, 9, 16, 25\}.}
		\subQuestion{Déclarer une chaîne de caractères initialisée à \og \textit{Trop bien le langage C} \fg{}.}
		\subQuestion{Déclarer une variable de type {\tt char} contenant le caractère '\$'.}
	\Question{Écrire les signatures des fonctions suivantes :}
		\subQuestion{{\tt divisible\_par} qui prend en argument deux entiers $n$ et $p$ et renvoie un booléen.}
		\subQuestion{{\tt somme} qui prend en argument un tableau de flottants et un entier et renvoie un flottant.}
		\subQuestion{{\tt carre} qui prend en argument un entier et renvoie un entier.}
		\subQuestion{{\tt affiche} qui prend en argument un booléen et ne renvoie rien.}
\end{Exercise}

\begin{Exercise}[title = {Boucles {\tt for}}]\\
...
\begin{Exercise}[title={Opérations sur les types de base}]
	...
	\Question{Écrire une expression booléenne valant vrai si {\tt a} est égal à {\tt b+c} ou si {\tt c} est non nul et que {\tt a} est divisible par {\tt c}. L'évaluation de cette expression génère-t-elle une erreur si {\tt c} est nul ?}
...
\begin{Exercise}[title={À propos de l'opérateur {\tt ++}}]
...
Que vaut {\tt b} si on a écrit \mintinline{c}{int b = a++ + a;} ? \\
Détailler votre raisonnement.}
\end{Exercise}

\begin{Exercise}[title={Chasse aux bugs}]\\
Dans le programme suivant, repérer les 4 anomalies (erreurs de syntaxe, avertissements du compilateur ou comportements indéfinis) et expliquer comment les corriger :
\begin{langageC}
#include <stdio.h>
