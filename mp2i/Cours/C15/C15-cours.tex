		...
		\begin{enumerate}
			\item<1-> On note $k = \left\lfloor \frac{n}{2} \right\rfloor$.
				\begin{itemize}
					...
					\item<3-> \textcolor{blue}{Régner} : on recherche récursivement les tranche maximales des sous tableaux $T_g$ et $T_d$ ainsi que celle des tranches contenant l'élément $t_k$.\\
						Afin de rechercher la valeur maximale d'une tranche contenant $t_k$, on peut rechercher le maximum des tranches se terminant en $t_k$ et le maximum des tranches commençant en $t_k$.
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Complexité}
		...
		\onslide<4->{A noter qu'à cause  des tailles respectives des \textit{facteurs cachés} dans l'algorithme naif et dans l'algorithme de Strassen, ce dernier ne devient plus efficace en terme de temps de calcul que pour de grandes valeurs de $n$.}
		\onslide<5->{
			\begin{center}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Correction}
		\addtocounter{enumi}{1}
		\begin{enumerate}
			\item<1->\textcolor{OliveGreen}{\small Pour tester chaque possibilité on doit énumérer tous les chemins possibles depuis le sommet vers un point de la base, en notant $n$ le nombre de niveau, on a deux choix à chaque niveau (excepté le premier), le nombre de chemins est donc $2^{n-1}$. La complexité est donc en $\mathcal{O}(n2^n)$ car il faut aussi calculer la somme de chaque chemin ($\mathcal{O}(n)$). Cette méthode n'est donc pas utilisable pour des valeurs de $n$ importantes.}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Chevauchement de sous problème}
		\begin{center}
			\begin{pspicture}(0,-2.7)(6,0.5) % Ajuster la taille de la figure selon les besoins
				...
				\psline[linecolor=BrickRed, linestyle=dashed,linewidth=0.5pt](2.5,0.2)(0.1,-2.7)(5.2,-2.7)(2.5,0.2)
				\psline[linecolor=BrickRed, linestyle=dashed,linewidth=0.5pt](3.5,0.2)(0.9,-2.7)(6.2,-2.7)(3.5,0.2)
			\end{pspicture}
		\end{center}
		Le sous problème de la pyramide \textcolor{OliveGreen}{verte} apparaît dans la résolution du sous problème de la pyramide droite \textit{et} celui de la pyramide gauche.
	\end{exampleblock}
\end{frame}

\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Résolution}
		En supposant la pyramide représentée par un tableau de tableau en OCaml, écrire la fonction {\tt smax} qui prend en argument une pyramide et renvoie sa somme maximale.
		\onslide<2->
		\inputpartOCaml{\SPATH/pyramide.ml}{}{}{1}{8}
	\end{exampleblock}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Complexité}
		\begin{itemize}
			...
			\item<1-> Sans mémoïsation, l'algorithme a une complexité \textit{exponentielle}, en effet la résolution d'un problème de taille $n$ demande la résolution de deux sous problèmes de taille $n-1$, c'est à dire qu'on a les équation de complexité : \\
			      $\left\{
				      \begin{array}{lll}
					      C(1) & = & 1            \\
					      C(n) & = & 2C(n-1)  + 1 \\
				      \end{array}
				      \right.$\\
			\item<2-> En mémoïsant les solutions des sous problèmes déjà traités (ou en procédant itérativement bas en haut), on obtient une complexité \textit{quadratique}, en effet chaque sous problème est résolu une unique fois. Or il y  a $\dfrac{n(n+1)}{2}$ sous problèmes et leur résolution s'effectue en temps constant (on compare simplement les valeurs des deux sous problèmes gauche et droit).
		\end{itemize}
	\end{exampleblock}
\end{frame}
