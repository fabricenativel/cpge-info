...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Bref historique}
		\begin{itemize}
			...
			\item<4-> À partir de 1983 : développement de plusieurs dérivés de C, parmi lesquels C++ (B. Stroustrup, 1983), C\# (Microsoft, 2000), Go (Google, 2007), Rust (Mozilla, 2010)
		\end{itemize}
	\end{block}
\end{frame}
	...
	\begin{block}{Compilation}
		...
		\begin{enumerate}
			...
			\item<4-> La compilation peut produire des erreurs ou des avertissements (\textit{warning}) \\
				\textcolor{gray}{\small La compilation se déroule en 4 étapes : préprocesseur, compilation, assemblage, édition des liens}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Programme minimal}
		\begin{overprint}
			...
			\medskip
			Les blocs d'instructions sont délimités par des accolades : \kw{\{} et \kw{\}}, ici c'est donc le bloc d'instruction de la fonction \kw{main}. À noter que les espaces, sauts de ligne et indentation sont ignorés par le compilateur, mais sont nécessaires pour une bonne lisibilité.
			\onslide<5>
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple de boucle}
		...
		\medskip
		Une variable dont la valeur ne sera pas modifiée peut être déclarée  en faisant précéder son type de \kw{const}.\\
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple de fonction et d'instruction conditionnelle}
		...
		\medskip
		{\small Appel à la fonction \kw{syracuse}, c'est une \textit{copie} du {\tt n} défini dans le \kw{main} qui est envoyé à la fonction, elle renvoie un entier.}
		\end{overprint}
	\end{exampleblock}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{alertblock}{Types de base}
		\begin{tabularx}{\linewidth}{|l|p{1.8cm}|>{\footnotesize}X|}
			...
			\hline
			\leavevmode\onslide<8-> {\kw{float} et \kw{double}}                   & \leavevmode\onslide<8->{\kw{+}, \kw{-}, \kw{*}, \kw{/}}                                                                            & \leavevmode\onslide<8->{Représentation des nombres en virgule flottante en simple ou double précision de la norme {\sc ieee754}. Fonctions élémentaires dans \kw{math.h}} \\
			\hline
			\leavevmode\onslide<9-> {\kw{bool} : \kw{true} ou \kw{false}.}                                   & \leavevmode\onslide<9->{\kw{||},  \kw{\&\&}, \kw{!}}                                                                                & \leavevmode\onslide<9->{Booléens accessibles dans \kw{stdbool.h}. Évaluation \og{}\textit{séquentielle}\fg{} des expressions.}                                                                 \\
			\hline
			\leavevmode\onslide<10-> {\kw{char}}                                   & \leavevmode\onslide<10->{\textcolor{gray}{\tt +, -}}                                                                                & \leavevmode\onslide<10->{Caractères notés entre quotes : \kw{'}, uniquement ceux de la table {\sc ascii}. Caractère nul : \kw{'\textbackslash{}0'}}                           \\
			\hline
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{\textcolor{Yellow}{\small \danger \; } Quelques points de vigilance !}
		\begin{itemize}
			\item<2-> L'opérateur de division {\tt /} sur deux entiers renvoie le \textcolor{blue}{quotient} dans la division euclidienne de ces deux entiers. Cela correspond donc à l'opérateur {\tt //} de Python.
			\item<3-> Pour obtenir le résultat de la division décimale, l'un au moins des opérandes doit être du type {\tt float}.
			\item<4-> Contrairement à Python, les entiers ont une taille maximale en C, et certaines opérations peuvent produire des \textit{dépassements de capacités}. Ce point sera étudié plus loin dans le cours.
			\item<5-> Le type \kw{char} ne permet de représenter qu'un unique caractère. Les chaînes de caractères seront vues plus loin dans le cours.
			\item<6-> Afin d'utiliser la bibliothèque \kw{math.h}, on doit ajouter l'option de compilation \kw{-lm}.
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemples}
		\begin{enumerate}
		\item<1-> Déclarer deux variables entières \kw{a} et \kw{b} initialisées respectivement à 2024 et 137. Déclarer \kw{c} ayant comme valeur le reste dans la division euclidienne de \kw{a} par \kw{b}.
		...
		\begin{itemize}
			...
			\item<2-> La portée peut être :
				\begin{itemize}
					...
					\item<4-> \textcolor{blue}{locale} lorsque la variable est déclarée dans un bloc d'instructions alors sa portée est limitée à ce bloc. C'est le cas des paramètres d'une fonction ou d'une variable de boucle.
				\end{itemize}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Remarques}
		...
		\begin{itemize}
			\item<2->\kw{-Wall} affichage de tous les \textit{warnings}
			\item<3->\kw{-Wextra} affichage de \textit{warnings} supplémentaires
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	...
	\begin{itemize}
		\item<4-> il ne faut \textcolor{red}{\sc pas} mettre de {\kw ;} après la condition (la syntaxe est valide mais signifie alors qu'il n'y a pas d'instructions à exécuter)
		\item<5-> le test d'égalité est le double égal \kw{==}, la construction avec un \kw{=} est valide, mais ne produit pas le résultat espéré !
		\item<6-> on recommande fortement (même si cela est possible) de ne pas se passer des accolades lorsqu'elles contiennent une unique instruction.
		\item<7-> on recommande aussi de ne \textit{pas utiliser} des entiers comme booléens, ni l'inverse. Ainsi si {\tt n} est un entier, l'écriture \mintinline{c}{if (n)}, bien que valide, est à proscrire, on écrira \mintinline{c}{if (n!=0)}.
	\end{itemize}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemples}
		Écrire les instructions conditionnelles suivantes (les variables utilisées sont supposées déjà déclarées):
		\begin{enumerate}
			...
			\item<4->{Écrire une fonction \kw{compare} en C, prenant comme paramètre deux entiers {\tt a} et {\tt b} et renvoyant {\tt -1} si {\tt a<b}, {\tt 0} si {\tt a=b} et 1 sinon.}
			\item<5-> Les deux instructions suivantes sont-elles \textit{rigoureusement} équivalentes ? : \\
			\mintinline{c}{if (d!=0 && n/d > 42) { a = 53;}} \\
			\mintinline{c}{if (n/d > 42 && d!=0) { a = 53;}} \\
		\end{enumerate}
	\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{alertblock}{Boucles}
		\begin{itemize}
			...
			permet de répéter le bloc d'instructions pour chaque valeur prise par la variable de boucle.
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple}
		...
		\begin{itemize}
			\item<2-> Écrire une boucle \kw{for} permettant d'afficher ces caractères.
...
\makess{Tableaux à une dimension, chaînes de caractères}
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Tableaux}
		...
		\begin{itemize}
			...
			\item<3-> On peut éventuellement initialiser  le tableau en donnant une liste de valeurs entre accolades \textcolor{blue}{\textit{--seulement en même temps que sa déclaration mais pas après--}} \\
			...
			\item<6-> On accède à un élément en donnant son numéro (son indice) entre crochets.\\
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple}
		\onslide<1-> Écrire une fonction \kw{croissant} qui prend en argument un tableau et sa taille et renvoie \kw{true} si le tableau est trié et false sinon.
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple}
		\onslide<1-> Écrire une fonction \kw{echange} qui prend en argument un tableau et deux indices  \kw{i} et \kw{j} ne renvoie rien et échange les éléments d'indice \kw{i} et \kw{j} de ce tableau.\\
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Chaînes de caractères}
		\begin{itemize}
			\item<1-> En C, les chaînes de caractères (notées entre guillemets \kw{"}) sont des tableaux de caractères (type \kw{char[]}) dont le dernier élément est le caractère spécial \kw{'\textbackslash{}0'} qui marque la fin de la chaîne.
			...
			\item<3-> Le module \kw{string.h} fournit des fonctions usuelles de manipulation de caractères, notamment :
				\begin{itemize}
					\item<4-> \kw{strlen} : renvoie la longueur de la chaîne de caractères
					\item<5-> \kw{strcpy} : copie une chaîne de caractères
					\item<6-> \kw{strcat} : concaténation de chaînes de caractères
				\end{itemize}
			\item<7-> Contrairement à un tableau \og{}classique\fg{}, on peut donc connaître la longueur d'une chaîne de caractères, grâce à la présence du caractère sentinelle \kw{'\textbackslash{}0'} qui en indique la fin.
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Définition}
		...
		\textcolor{BrickRed}{\small \danger } Quand un tableau multidimensionnel est passé à une fonction, toutes les dimensions sauf (éventuellement) la \textit{première} doivent figurer ! :
		Par exemple, la signature de la fonction suivante n'est \textit{pas} correcte : \\
		\mintinline{c}{void affiche(bool m[][], int l, int c)} \\
		On devrait écrire : \\
		\mintinline{c}{void affiche(bool m[][10], int l, int c)}
	\end{block}
\end{frame}

\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple}
		Écrire en C une fonction de signature \mintinline{c}{void identite(int n)} qui affiche la matrice identité d'ordre {\tt n} (c'est-à-dire qu'elle contient des 1 sur sa diagonale principale et des 0 partout ailleurs). On déclarera un tableau à deux dimensions dans la fonction et on le remplira correctement avant d'effectuer son affichage.
	\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	...
	\begin{exampleblock}{Exemple}
		\begin{itemize}
			\item<6-> Écrire un programme qui demande à l'utilisateur de saisir au clavier deux entiers {\tt a} et {\tt b} puis affiche leur somme.
