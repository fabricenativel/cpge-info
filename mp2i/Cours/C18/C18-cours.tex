...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple}
		...
		\textbf{\texttt{Patient}} {\tt (\underline{idpatient}, nom, prenom, age, \#id\_medicament)} \\
		\textbf{\texttt{Médicament}} {\tt (\underline{id}, dénomination, dosage, duree, molécule)} \\
		L'attribut {\tt id\_medicament} est une clé étrangère qui référence la clé primaire id de la table Médicament.
	\begin{itemize}
	\item Est-il possible d'associer plusieurs médicament à un même patient avec un schéma ?
	\item Une durée de traitement est souvent propre à un patient, est-ce la cas ici ?
	\item Résoudre les deux problèmes précédents en créant un nouvelle table {\tt prescriptions} et donenr alors nouveau schéma relationnel de la base de données.
	\end{itemize}
	\end{exampleblock}
\end{frame}
