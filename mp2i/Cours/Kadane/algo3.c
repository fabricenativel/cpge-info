...
{
    // Tableau a un élément : la tranche maximale est égale à l'élément
    if (end==start) return tab[start];
    // Tableau a deux éléments : c'est le max entre la tranche et les 2 éléments
    if (end==start+1) return max3(tab[start],tab[end],tab[start]+tab[end]);
    // La tableau a au moins 3 éléments
    int s1, s2, s3;
    int mid = (start+end)/2;
