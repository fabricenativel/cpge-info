...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Exercices}
		\begin{enumerate}
			\item<1-> Utiliser {\tt List.iter} pour afficher les mots de 5 lettres d'une liste de mots.
			\item<2-> Ecrire, \textit{en utilisant} \mintinline{ocaml}{List.iter}, une fonction {\tt somme: int list -> int} qui renvoie la somme des éléments de la liste d'entiers donnée en argument. \\
			\textcolor{OliveGreen}{\small \aide \;} Utiliser les aspects impératifs du langage
		\end{enumerate}
	\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Analyse de l'algorithme {\sc bfs}}
		\begin{itemize}
			...
			\item<3-> Pour la \textcolor{blue}{correction}, on note $G = (V,E)$ le graphe, $s \in V$ le sommet source, $d[v]$ le tableau des distances et $Q$ la file et $\delta(v)$ la distance minimale du sommet source $s$ au sommet $v$. Puis on prouve : 
			\begin{itemize}
				\item<4-> L'invariant noté $(I)$ : \og{}\textit{La file $Q$ contient $k$ sommets $v_1, \dots v_k$ avec $d[v_1] \leqslant \dots \leqslant d[v_k]$ et $d[v_k] \leqslant d[v_1] +1$} \fg{} (envisager l'évolution de la file suivant les cas enfiler ou défiler). 
				\item<5->Et par récurrence sur la propriété $\mathcal{P}(n)$ : \og{}\textit{pour tout sommet $v$ à distance $n$ de $s$, on a $d[v]=\delta(v)$} \fg{}.
			\end{itemize} 
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	...
	\onslide<4->{\begin{alertblock}{Définition}
		...
		\onslide<5->{\textcolor{gray}{Pourquoi précise-t-on que le graphe est acyclique ?}}
		\end{alertblock}
	}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Exercice}
	On suppose (dans le langage de votre choix), un graphe représenté par sa matrice d'adjacence. Ecrire une fonction qui prend en argument un graphe (à $n$ sommets) ainsi qu'un tableau à $n$ éléments et renvoie un booléen indiquant si ce tableau est un tri topologique valide ou non de ce graphe.
	\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	...
	\onslide<5->{
	\begin{exampleblock}{Exemple}
		Dérouler soigneusement l'algorithme {\tt dfs} sur le graphe suivant \textit{pour chaque sommet} de $s_0$ à $s_5$. Quel est l'ordre postfixe obtenu ?
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Principe}
		\begin{itemize}
		\item<1-> L'algorithme de Dijkstra recherche le plus court chemin depuis un sommet de départ $s$ vers \textit{tous les autres} sommets du graphe.
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Proposition d'implémentation}
		\inputpartOCaml{\SPATH/dijkstra.ml}{}{\small}{64}{78}
	\end{block}
\end{frame}
