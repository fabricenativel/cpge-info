...
\begin{frame}[fragile]{\Ctitle}{\stitle}
    \begin{block}{Remarques}
        Ecrire $u_n = O(v_n)$ traduit une \textit{majoration asymptotique}, \\
        \textcolor{gray}{Par exemple si $u_n = 42n+2024$,  on pourrait écrire $u_n = O(n)$ mais aussi  que $u_n = O(n^2)$ (ou encore $u_n = O(n^3)$).}\\
        On veut généralement  donner le \og{} meilleur grand O \fg{}. Afin d'exprimer formellement cette notion, on note :
        \begin{itemize}
            \item<2-> $u_n = \Omega(v_n)$ s'il existe $K \in \R^+$ et $n_0 \in \N$ tel que pour tout $n \geqslant n_0, u_n \geqslant Kv_n$ \\
            \item<3-> $v_n = \Theta(v_n)$ si $u_n = O(v_n)$ et $v_n = O(u_n)$ \\
        \end{itemize}
        \onslide<4->{On utilisera principalement la notation $O$, en gardant à l'esprit qu'on essaye toujours d'avoir la meilleure majoration}.
    \end{block}
\end{frame}
