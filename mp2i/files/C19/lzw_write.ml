...
let write_code writer c =
  let a = c/256 in
  let b = c mod 256 in
  Printf.fprintf writer "%c" (char_of_int a);
  Printf.fprintf writer "%c" (char_of_int b);;

  let ecrire fname res =
    let writer = open_out fname in
    Printf.fprintf writer "%s" res;;

let ecrire_comp fname lcode =
  let writer = open_out fname in
  List.iter (fun x -> write_code writer x) lcode;;
...
let () =  let ascii = cree_code_ascii() in
          ...
          ecrire_comp "texte_compresse.txt" tc;
