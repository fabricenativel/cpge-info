...
int get_value_hashtable(list hastable[SIZE], char w[26])
{
    int bucket_number = hahsword(w) % SIZE;
    return get_value_list(hastable[bucket_number], w);
}
...
int main()
{
    ...
    while ((word = get_word(reader)) != NULL)
    {
        ...
        {
            old_occ = get_value_hashtable(dict, word);
