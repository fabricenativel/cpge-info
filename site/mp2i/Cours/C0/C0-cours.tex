...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Modèle de Von Neumann}
		\begin{itemize}
			...
			\item<2-> Dans ce modèle, l'ordinateur se décompose en :
				\begin{itemize}
					\item<3-> La \textcolor{blue}{mémoire} qui stocke les données (sous forme de 0 et de 1).
					...
					\item<5-> L'\textcolor{blue}{unité de contrôle} chargée de l'ordre des opérations et de la récupération des données en mémoire.
...
\begin{frame}{\Ctitle}{\stitle}	
	\begin{alertblock}{Définition}
		Un \textcolor{red}{système d'exploitation} (en abrégé \textcolor{red}{OS}, de l'anglais \textit{Operating System}) est un programme (ou ensemble de programmes) permettant de
		\onslide<2->{gérer les ressources de l'ordinateur (mémoire, fichiers, périphériques, \dots) sur lequel il s'exécute}. \\
	\end{alertblock}
	\onslide<3->{\begin{exampleblock}{Exemples}
			...
			\begin{itemize}
				...
				\item[\faLinux] <5-> {\sc gnu}/Linux (plusieurs centaines de distributions différentes, parmi les plus connues : Ubuntu, Fedora, Arch Linux)
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{La place du système d'exploitation}
		...
		\onslide<8->{\naput{\textit{\small gère les ressources de		}}}
	\end{block}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Fonctionnalités d'un système d'exploitation}
		\begin{itemize}
			...
			\item<6-> Gérer le système de fichiers.
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}	
	\begin{block}{Le système de gestion de fichiers}
		\begin{itemize}
			...
			\item<3-> Les fichiers ou les répertoires sont spécifiés :
				\begin{itemize}
				...
				\end{itemize}
			\item<6-> Trois types de droits sont définis sur les fichiers et dossiers :
				\begin{itemize}
					\item<6->[{\tt r}] droit de lecture du fichier
					\item<7->[{\tt w}] droit d'écriture  dans le fichier
					\item<8->[{\tt x}] droit d'exécution du fichier
				\end{itemize}
		\end{itemize}
	\end{block}
\end{frame}
	...
	\begin{block}{Interface système : \textit{shell}}
		\begin{itemize}
			...
			\item<3-> Voici un exemple d'invite de commande :\\
				...
				On y trouve :
				\begin{itemize}
					\item<4-> Le nom de l'utilisateur ici \textcolor{OliveGreen}{\tt neo} suivi de \textcolor{OliveGreen}{\tt @}
					\item<5-> Le nom de l'ordinateur ici \textcolor{OliveGreen}{\tt matrix} suivi de {\tt :}
					\item<6-> Le chemin du dossier de travail ici \textcolor{blue}{\tt \~{}/Morpheus}  et enfin le caractère d'invite ({\tt \$} pour un utilisateur et {\tt \#} pour root).
				\end{itemize}
		\end{itemize}
	\end{block}
\end{frame}
		...
		\begin{itemize}
			...
			\item<2-> Certains caractères spéciaux permettent d'agir sur un ensemble d'arguments : \\
				\begin{tabularx}{0.8\linewidth}{|c|X|}
					...
					\hline
					\textcolor{Sepia}{\tt *}    & correspond à n'importe quelle suite de caractères            \\
					\hline
			...
			\item<3-> Le résultat d'une commande peut être dirigé :
				...
				\end{itemize}
			\item<7-> Les touches de direction (\UArrow et \DArrow) permettent de circuler dans l'historique des commandes, la touche de tabulation (\Tab) permet de compléter automatiquement les noms de fichiers.
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemples}
		\tei{ExArbo.eps}{0.4}{1}{
		À partir du répertoire {\tt Cours}, écrire les commandes pour :
		\begin{enumerate}
			\item<1-> Créer le fichier vide {\tt to\_do} \\
				\onslide<2->{\textcolor{OliveGreen}{\tt touch to\_do}}
			\item<3-> Écrire dans ce fichier : "{\tt réviser le chapitre 0}" \\
			...
			\item<9-> Lister tous les fichiers contenant {\tt edf}\\
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Droits sur un fichier, {\tt chmod}}
		\begin{itemize}
			\item<2-> Les droits sur un fichier (\textcolor{blue}{\tt r} : lecture, \textcolor{blue}{\tt w} : écriture, \textcolor{blue}{\tt x} : exécution) sont définis pour le propriétaire : {\textcolor{blue}{\tt u}} (\textit{user}), le groupe : {\textcolor{blue}{\tt g}} (\textit{group}) et les autres : {\textcolor{blue}{\tt o}} (\textit{others}).
			\item<3-> La commande \textcolor{blue}{\tt ls -l} permet d'afficher les droits, par exemple : \\
				{\tt rwxr-x---} indique que l'utilisateur a tous les droits, le groupe peut lire et exécuter et les autres n'ont aucun droit.
	...
	\begin{exampleblock}{Exemples}
		\begin{itemize}
			\item<5->{{\tt chmod u+w } :} \onslide<6->{\footnotesize Ajoute ({\tt +}) le droit d'écriture ({\tt w}) au propriétaire ({\tt u}) }
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Flux standards}
		Sous un système Linux/Unix, trois canaux de communication appelés flux standards sont définis par défaut pour un programme :
		\begin{itemize}
			\item<2-> \textcolor{blue}{stdin} (pour \textit{Standard Input}), le canal d'entrée des données fournies au programme qui est par défaut le clavier (descripteur {\tt 0}).
			\item<3-> \textcolor{blue}{stdout} (pour \textit{Standard Output}), le canal de sortie des résultats du programme qui est par défaut l'écran (descripteur {\tt 1}).
			\item<4-> \textcolor{blue}{stderr} (pour \textit{Standard Error}), le canal de sortie des erreurs du programme qui est par défaut l'écran (descripteur {\tt 2}).
		\end{itemize}
	\onslide<5-> On peut vouloir, dans certaines situations, rediriger ces flux de données vers des sorties ou entrées plus adaptées. Quelques exemples sont donnés sur la diapositive suivante.
	\end{block}
\end{frame}

\begin{frame}{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemples de redirection des flux standards}
		\begin{itemize}
		\item<2-> Les résultats d'une commande et ses erreurs éventuelles s'affichent tous les deux dans le même terminal, on peut vouloir séparer les deux. Par exemple la commande \kw{find} permet de rechercher des fichiers sur l'ordinateur mais elle peut générer de nombreux messages d'erreurs (\textit{permission denied}) lorsqu'elle ne peut pas accéder à un répertoire. Rediriger \textit{stderr} permet alors d'afficher uniquement les lignes pertinentes. On écrirait alors une commande telle que : {\tt find [options diverses] 2> erreurs\_find.txt}. 
		\item<3-> Plutôt que de fournir des données à un programme via le clavier (ce qui peut être fastidieux), on peut stocker ces données dans un fichier et rediriger l'entrée standard vers ce fichier, ce qui donnerait : {\tt ./mon\_programme < test.txt}.
		\end{itemize}
	\end{exampleblock}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Liens physiques ou symboliques}
		\begin{itemize}
			\item<1-> Les fichiers sont stockés sur le support physique par blocs et retrouvés grâce à leur \textcolor{blue}{inode} (\textit{index node} ou nœud d'index).
