			...
			\item<3-> Le schéma ci-dessous représente un pointeur \kw{p} contenant l'adresse d'une variable \kw{x} qui vaut 42. \\
				\onslide<4->{
					...
					\begin{tabular}{ccccccccc}
						...
						\textcolor{OliveGreen}{type} &       &                                      & \textcolor{OliveGreen}{\tt int}                                                      &                                      &       &       & \textcolor{OliveGreen}{\tt int *}                                          &       \\
						\cline{3-5} \cline{8-8}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Schéma de l'organisation de la mémoire en C}
		...
		\rput(9.5,2.2){\parbox{6cm}{\center \scriptsize Taille variable \\ Allouée dynamiquement par le programmeur avec \kw{malloc} et libérée avec \kw{free} }}
	\end{block}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Conséquences}
		...
		\begin{itemize}
			...
			\item<3-> De la mémoire allouée par le programmeur dans le tas et non libérée est considérée \textcolor{blue}{comme non disponible}, créant des fuites mémoires (\textit{memory leak}).
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Un  exemple}
		\inputpartC{\SPATH/varlocation.c}{}{\small}{3}{17}
	\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Remarque}
		Lorsqu'on déclare un pointeur vers un entier par exemple avec \mintinline{c}{int * p;}, on peut considérer que :
		\begin{itemize}
		\item<2-> \kw{p} est une variable de type \kw{int*} qui contient l'adresse d'un entier :\\
		\rnode{A}{\boxed{{\tt \textcolor{BrickRed}{int}{\tt *}}}} \ \ \ \rnode{B}{\tt p} \ncbar[angleA=270]{->}{B}{A}  : \textcolor{blue}{\textit{{\tt p} est l'adresse d'un {\tt int}}} \medskip
		\item<2-> \kw{*p} est une variable de type \kw{int} qui contient la valeur de l'entier pointé par \kw{p} :\\
		\rnode{C}{\boxed{{\tt \textcolor{BrickRed}{int}}}} \ \ \ \rnode{D}{\tt *p} \ncbar[angleA=270]{->}{D}{C}  : \textcolor{blue}{\textit{{\tt *p} est un {\tt int}}} \medskip
		\end{itemize}
	\onslide<3->{On peut donc manipuler {\tt p} :
	\begin{itemize}
	\item En lui attribuant l'adresse d'un entier, par exemple avec \mintinline{c}{p = &n} où \kw{n} est un entier.
	\item En affectant un {\tt int} à {\tt *p}, \textcolor{BrickRed}{\textit{à condition que l'espace mémoire pointé par p soit alloué}}. Par exemple avec \mintinline{c}{*p = n;} où \kw{n} est un entier.
	\end{itemize}}
	\end{block}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{alertblock}{{\tt malloc}}
		\begin{itemize}
			...
			\mintinline{c}{int *t =  malloc(sizeof(int)*n); //n entier positif}:\\
			\rnode{A}{\tt t} \quad \renewcommand{\arraystretch}{0.9} \begin{tabular}{|c|c|c|c|c|c|}
				\hline
			...
			\end{tabular} \\ \medskip
			\ncline[linewidth=0.04, linecolor=BrickRed,nodesepB=0.1]{->}{A}{B}
			\ncline{<->,nodesep=-0.5}{C}{D} \nbput[labelsep=0.05]{\scriptsize{\tt sizeof(int)}}
			\textcolor{gray}{\quad \small \important \ Une syntaxe équivalente pour accéder à \kw{t[i]} est \kw{*(t+i)}.}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Quelques exemples}
		...
		\inputpartC{\SPATH/exmalloc.c}{}{\footnotesize}{6}{6}
		Le pointeur \kw{ptr1}, est \textit{non initialisé}, le déréférencer, par exemple pour lui attribuer une valeur (avec \mintinline{c}{*ptr1=42;}) est un comportement indéfini.
		\item<2-> Pointeur initialisé à {\sc null}
		\inputpartC{\SPATH/exmalloc.c}{}{\footnotesize}{7}{7}
		Le pointeur \kw{ptr2}, est initialisé à {\sc null}, son déréférencement est un comportement indéfini. 
		\item<3-> Pointeur initialisé avec l'adresse d'une variable locale.
		\inputpartC{\SPATH/exmalloc.c}{}{\footnotesize}{8}{9}
		Le pointeur \kw{ptr3}, est initialisé avec l'adresse de {\tt n} (situé dans la pile). Le déréférencement de \kw{ptr3} donne la valeur de {\tt n} (ici 42).
		\end{itemize}
	\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Quelques exemples}
		\begin{itemize}
			...
			\inputpartC{\SPATH/exmalloc.c}{}{\small}{10}{13}
			Le pointeur \kw{ptr4}, est initialisé avec l'adresse d'un bloc mémoire alloué dynamiquement. Ce bloc mémoire est situé dans le \textcolor{BrickRed}{tas}, cet espace mémoire est disponible pour y stocker un entier.
		\end{itemize}
	\end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{alertblock}{{\tt free}}
		\begin{itemize}
			...
			\item<2-> On appelle donc \kw{free} sur un pointeur \kw{p} créé par \kw{malloc}. \\
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Conséquences}
		En C, on n'écrira pas \textit{à proprement parler} de fonctions renvoyant un tableau, mais plutôt des fonctions renvoyant un pointeur vers un bloc mémoire alloué dynamiquement que l'on traitera \textit{comme un tableau}. \\
		\onslide<2->{Il faut cependant respecter certaines règles :}
		\begin{itemize}
			\item<3->[\textcolor{BrickRed}{\small \danger}]  L'allocation \textcolor{BrickRed}{doit} se faire avec \kw{malloc}, sinon le bloc mémoire est alloué sur la pile, dans la zone de contexte d'appel de la fonction, cette zone est dépilé dès qu'on quitte la fonction et donc la zone mémoire du tableau n'est plus accessible.
			\item<4->[\textcolor{BrickRed}{\small \danger}] La fonction appelante doit libérer la mémoire allouée avec \kw{free} (sous peine de fuites mémoires.)
		\end{itemize}
		\onslide<5->{D'autre part, on s'autorisera dans les exercices et à l'oral l'abus de langage \og{}\textit{renvoyer un tableau}\fg{}. Il faudra garder à l'esprit qu'on renvoie en fait un pointeur !}
	\end{block}
\end{frame}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Exemple}
		...
		\onslide<2->{\inputpartC{\SPATH/renvoie_tab.c}{}{\footnotesize}{3}{18}}
	\end{exampleblock}
\end{frame}

\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Tableaux et pointeurs}
		\textcolor{BrickRed}{\small \danger \;} En C, la confusion entre tableaux et pointeurs est fréquente et on a tendance à considérer que des écritures comme \mintinline{c}{int t[10];} et \mintinline{c}{int *t = malloc(sizeof(int)*10);} sont équivalentes. Bien qu'on puisse utiliser dans les deux cas la syntaxe \mintinline{c}{t[i]} (avec $0\leqslant i < 10$), c'est \textcolor{BrickRed}{faux} ! Notamment   :
		\begin{itemize}
			\item<2-> Dans un cas \kw{t} est alloué sur la pile, dans l'autre sur le tas.
			\item<3-> La fonction \mintinline{c}{sizeof} ne renvoie pas la même valeur dans les deux cas. En particulier il est possible d'obtenir la taille du tableau, pas celle du bloc mémoire.
		...
		\end{itemize}
		\onslide<5->{Cependant, un tableau passé en argument à une fonction est traité comme un pointeur (par un mécanisme appelé \textit{pointer decay}).}
	\end{block}
\end{frame}
...
\begin{frame}{\Ctitle}{\stitle}
	\begin{block}{Arguments de {\tt main}}
		\begin{itemize}
			...
			\item<2-> Ces arguments doivent alors être fournis à l'exécutable lors de son exécution.
			\item<3-> Ces arguments sont traités comme des chaines de caractères et doivent donc être convertis dans le type adéquat si besoin grâce aux fonctions suivantes disponibles dans \kw{stdlib.h}.
				\begin{itemize}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Définitions}
		\begin{itemize}
			\item<1-> On peut définir en C, des types structurés, appelés \kw{struct} et composés de plusieurs champs.
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{exampleblock}{Déclaration, lecture et écriture d'un champ}
		\begin{itemize}
			\item<1-> La déclaration d'une variable de type personne peut se faire maintenant avec : \\
				\mintinline{c}{personne bruce_banner;}
			\item<2-> Eventuellement avec initialisation immédiate avec la notation \kw{\{} et \kw{\}} déjà rencontrée sur les tableaux : \\
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{alertblock}{Méthode}
		\begin{itemize}
			...
			\item<3-> Les corps des fonctions sont écrits dans {\tt <module.c>} et ce fichier est compilé de façon à obtenir un fichier objet \kw{module.o} grâce à l'option \kw{-c} de {\tt gcc}.
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{Remarques}
		\begin{itemize}
			...
			\item<3-> Afin d'éviter des conflits entre des fonctions portant le même nom et définies dans des modules ou dans le programme principal, on peut préfixer tous les noms de fonctions ou de type d'un module par le nom de ce module.
		\end{itemize}
	\end{block}
\end{frame}
	...
	\begin{block}{Allocation de l'espace mémoire}
		\begin{itemize}
			\item<1-> En C, on peut allouer des tableaux multidimensionnels sur la pile en précisant les tailles attendues pour chaque dimension. Par exemple :\\
				\onslide<2->{\mintinline{c}{float matrice[4][4];} \\déclare un tableau de flottants de 4 lignes sur 4 colonnes. L'accès aux éléments se fait alors avec la notation {\tt matrice[i][j]}.\\}
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{$V_0$ : Allocation d'un tableau à une dimension}
		\begin{itemize}
			\item<1-> On alloue un tableau d'entiers de tailles $n \times m$ et on considère que les $m$ premiers forment la première ligne, que les $m$ suivant sont la deuxième, \dots.
...
\begin{frame}[fragile]{\Ctitle}{\stitle}
	\begin{block}{$V_1$ :Allocation d'un tableau de tableaux}
		\begin{itemize}
			\item<1-> Un tableau d'entiers de type {\tt int} et de taille $n$ a la structure suivante en mémoire (où chaque case a la taille prévue pour accueillir un {\tt int})
				\begin{center}
					...
					\end{tabularx}
				\end{center}
				\item<2->{Pour allouer une matrice de dimension $n \times m$, on peut "linéariser" le tableau et prévoir que chaque case a maintenant la capacité d'accueillir $m$ entiers. \textit{c'est-à-dire que c'est la solution précédente mais avec une gestion automatique des indices par le langage C.}}
